...
export default function NotFoundSection({
  ...
  const ignoreSelected = () => {
    ...
    if (ignoreApproachIds.size) {
      setBadApproaches(
        (badApproaches as unknown as Array<string | number>).filter(
          (a) => !ignoreApproachIds.has(Number(a))
        ) as unknown as any
      )
    }
    ...
    if (ignoreDetChannels.size) {
      setBadDetectors(
        (badDetectors as unknown as Array<string | number>).filter(
          (d) => !ignoreDetChannels.has(d?.toString() ?? '')
        ) as unknown as any
      )
    }
